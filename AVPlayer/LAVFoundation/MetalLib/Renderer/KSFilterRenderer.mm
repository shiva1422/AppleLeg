...
{
    UIImage *image = [UIImage imageNamed:@"sample.jpeg"];//pixels are not directly accessed so below also flip texutre as metal texture starts from top.
    assert(image != nil);
...
-(void)updateTexture:(CVPixelBufferRef)pixelBuffer
{
    ...
    //from above loadTexture later use metalTextureCache; just temporary.
    ...
    //set Texture data.
    MTLRegion region = MTLRegionMake2D(0, 0, width, height);
    [_texture replaceRegion:region mipmapLevel:0 withBytes:pixels bytesPerRow:bytesPerRow];
    free(pixels);
